...
> ⚠️ **Этот файл устарел!** Актуальная документация: **[DATABASE.md](DATABASE.md#8-user_events-события-пользователей-374-фз)**
...
3. **Срок хранения** — **бессрочно** (данные НЕ очищаются)
4. **IP адрес** — определяется из X-Forwarded-For, X-Real-IP или RemoteAddr
5. **⚠️ user_events НЕ подлежит очистке!** — данные о регистрации и смене имён накапливаются бессрочно

### 🗑️ Очистка данных

**Важно:** В отличие от `chat_messages` (сообщения чата), таблица `user_events` **НЕ очищается**:
...
**Обоснование:**
- Сведения о регистрации и смене имён накапливаются бессрочно
- Данные необходимы для идентификации пользователей
- Предоставляются уполномоченным органам по запросу
...
- **chat_messages**: автоматическая очистка старше 1 года
