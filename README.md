...
Децентрализованный чат-сервис с системой задач и **real-time** обменом сообщениями. Веб-клиент с поддержкой мобильных устройств.
...
```
http://87.242.103.13:8080
```

**Развёртывание на Cloud.ru:** см. [DEPLOYMENT_CLOUD_RU.md](DEPLOYMENT_CLOUD_RU.md)
...
- **Сохранение сессии** — авторизация сохраняется после перезагрузки страницы (7 дней)
- **Мобильная адаптация** — корректно работает на смартфонах и планшетах
- **Тёмная тема** — серая цветовая схема
- **Автоматическое версионирование** — версия увеличивается при каждой сборке

## Цветовая схема

| Элемент | Цвет | HEX |
|---------|------|-----|
| Фон страницы | Чёрный | `#000000` |
...
| Кнопка SEND | Светлый фон/тёмный текст | `#e6e6e6` / `#2a2a2a` |
| Версия | Светло-серый | `#cccccc` |
...
└────────────┼────────────────────────────────────────────────────────────┘
    ...
    └───────────┬────────────┘
         ...
         ▼            ▼           ▼          ▼              ▼
   ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌────────────┐ ┌──────────────┐
   │  Auth   │ │  Chat   │ │  Task   │ │Notification│ │ Compliance   │
...
```
┌─────────────────────────────────────────────────────────────────┐
│  ☢ STALKnet  v0.1.3              Guest  ●                       │
└─────────────────────────────────────────────────────────────────┘
```

**Основная область:**
- Слева: список сообщений с timestamp
- Справа: индикатор подключения (●)
  - 🔴 Красный — нет подключения
  - 🟡 Жёлтый — гость
  - 🟢 Зелёный — авторизован и подключён
...
**Цветовая схема:**
- Фон: `#000000` (чёрный)
- Верхняя панель: `#3c3d4a` (серо-синий)
- Текст: `#e6e6e6` (светло-серый)
- Имя пользователя: `#cf9c4e` (золотистый)
- Сообщения: `#cdc0b0` (бежево-серый)
- Задачи: `#cf9c4e` (золотисто-оранжевый)

## Быстрый старт

### 🌐 Production-доступ

**Веб-клиент доступен по адресу:**

```
http://87.242.103.13:8080
```

**Группа безопасности Cloud.ru:** STALKnet-access
...
# 3. Запуск Web-клиента (в новом терминале)
cd client
go run main_web.go
```
...
.\scripts\bump_version.ps1    # Обновить версию
docker-compose build gateway  # Собрать образ
docker-compose up -d gateway  # Запустить контейнер
```
...
# Редактирование .env.production (измените JWT_SECRET и DB_PASSWORD)
...
```bash
# Измените домен в docker-compose.traefik.yml
docker-compose -f docker-compose.traefik.yml up -d
```

### Версионирование

Версия приложения отображается в заголовке справа: **STALKnet v0.1.3**
...
**Ручное обновление:**
```powershell
# Увеличить PATCH-версию (0.1.3 → 0.1.4)
...
| Скрипт | Назначение |
|--------|------------|
| `build.ps1` | Автоматическая сборка с обновлением версии |
| `bump_version.ps1` | Увеличить PATCH-версию на 1 |
| `update_version.ps1` | Умное обновление (minor/patch) |
| `update_version.sh` | Bash-версия для Linux/Mac |
| `fix_borders.ps1` | Исправить рамки в сообщениях |
